...
public class InvoiceServiceImpl implements InvoiceService {
    ...
    private final org.springframework.data.jpa.repository.JpaRepository<ProductStub, Long> productRepo;
    private final org.springframework.data.jpa.repository.JpaRepository<BranchInventoryStub, BranchInventoryId> inventoryRepo;
    private final org.springframework.data.jpa.repository.JpaRepository<BranchProductPriceStub, BranchProductPriceId> priceRepo;
    private final org.springframework.data.jpa.repository.JpaRepository<PromotionDetailStub, Long> promotionDetailRepo;
    private final org.springframework.data.jpa.repository.JpaRepository<InventoryTransactionHistoryStub, Long> ithRepo;
    ...
    public Page<ProductSearchResponse> searchProducts(String keyword, String sku,
        ...
        List<ProductStub> allProducts = productRepo.findAll();

        List<ProductStub> filtered = allProducts.stream()
    ...
    public InvoiceResponse addItem(Long invoiceId, AddInvoiceItemRequest request) {
        ...
        ProductStub product = productRepo.findById(request.productId())
    ...
    public InvoiceResponse payInvoice(Long invoiceId, PayInvoiceRequest request) {
        ...
        for (InvoiceDetail detail : invoice.getDetails()) {
            ...
            BranchInventoryStub inventory = entityManager.find(
                    BranchInventoryStub.class, invKey, LockModeType.PESSIMISTIC_WRITE);

            if (inventory == null || inventory.getQtyAvailable().compareTo(detail.getQuantity()) < 0) {
                ProductStub product = productRepo.findById(detail.getProductId()).orElse(null);
        ...
        for (InvoiceDetail detail : invoice.getDetails()) {
            BranchInventoryId invKey = new BranchInventoryId(invoice.getBranchId(), detail.getProductId());
            BranchInventoryStub inventory = entityManager.find(BranchInventoryStub.class, invKey,
            ...
            InventoryTransactionHistoryStub ith = InventoryTransactionHistoryStub.builder()
                    ...
                    .reason("POS Sale")
                    .createdBy(cashier.getEmployeeId())
                    .createdAt(now)
                    .build();
            ithRepo.save(ith);
        }
    ...
    private BigDecimal resolveEffectivePrice(ProductStub product, Integer branchId) {
    ...
    private InvoiceResponse buildFullResponse(Invoice invoice) {
        ...
        Map<Long, ProductStub> productMap = productRepo.findAllById(productIds).stream()
                .collect(Collectors.toMap(ProductStub::getProductId, p -> p));
    ...
    private ProductSearchResponse buildProductSearchResponse(ProductStub product, Integer branchId) {
        BigDecimal effectivePrice = resolveEffectivePrice(product, branchId);
        BranchInventoryStub inv = inventoryRepo
